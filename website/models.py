...
class Ledger(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id')) # I have no idea if this was neccessary, but from what I gather you would need to grab the user account for the ledger as well as the account.

    entry_desc = db.Column(db.String(150))

    entry_date = db.Column(db.DateTime(timezone=True), default=func.now()) #I made this to a date of creation function (I think) since thats the date that is wanted.
